...
const ShopItems = ({
  ...
  const getItemNumber = (product) => {
    if (product.specification) {
      return `${product.specification[0].number.split("-")[0]}-${
        product.specification[0].number.split("-")[1]
      }`;
    }
    return "";
  };
    ...
    <Container>
      ...
      <MenuContent>
        ...
        <Detail>
          ...
          <Number>{"품번"}</Number>
          <Classify>{"분류"}</Classify>
          <Specifications>{"품목수"}</Specifications>
        </Detail>
      </MenuContent>
      <ItemContent>
        {products.map(
                ...
                {product && (
                  <Content onClick={() => onClickProduct(product.id)}>
                    ...
                    <Detail>
                      ...
                      <Number>{getItemNumber(product)}</Number>
                      <Classify>{product.classify}</Classify>
                      <Specifications>
